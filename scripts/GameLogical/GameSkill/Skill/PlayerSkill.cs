...
namespace GameLogical.GameSkill{
	public class CPlayerSkill : CSkillBass
	{
		...
		/*	if(CLineSmoothMgr.GetInstance().mpStone < skillModuleData.greenStone){
				return false ;
			}

			EventMessageUseSkill useSkillMessage = new EventMessageUseSkill();
			useSkillMessage.id = EnitityMgr.GetInstance().palyer.GetId();
