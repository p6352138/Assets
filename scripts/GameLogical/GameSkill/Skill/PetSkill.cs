...
namespace GameLogical.GameSkill{
	public class CPetSkill : CSkillBass
	{
				...
				if(skillModuleData.range == (int)BuffRangeType.BUFF_RANGE_SINGLE){
					...
					if(skillModuleData.useObject == 1){
						...
						if(skillModuleData.range == 3){
							...
							int tempID = -1 ;
							float tempHp = float.MaxValue ;
							float tempPercent ;
							for(int i = 0; i<petList.Count; ++i){
								tempPercent = (float)petList[i].GetFightCreatureData().blood /  (float)petList[i].GetFightCreatureData().maxBlood;
								if(tempPercent < tempHp && petList[i].GetFightCreatureData().blood > 0){
									tempID = petList[i].GetId();
									tempHp = tempPercent;
								}
							}
...
=======
				...
				//range           
					...
					CCreature carryCreature = EnitityMgr.GetInstance().GetEnitity(m_data.carryID);
					if(skillModuleData.useObject == 1){
						...
						rangeBuff.destCreatures = EnitityMgr.GetInstance().FindAreaCreatureList(carryCreature,EnitityType.ENITITY_TYPE_PET,m_data.range) ;
					}
		...
		public void useAngrySkill(EventMessageEnititySelect eventData){
			...
			sceneOb2.transform.FindChild("creature").GetComponent<Bass2D.Amin_2D_Ex>().sortOrder = 3;
		...
		public void FinshLoadRangeBuffEffect(Object ob,MGResouce.LoadEffectData data){
			if(ob != null){
				...
				Transform animTran = sceneOb.transform.FindChild("creature");
				if(animTran != null){
					if(animTran.animation != null){
						animTran.animation.Play("effect");
					}
				}
			}
		}
