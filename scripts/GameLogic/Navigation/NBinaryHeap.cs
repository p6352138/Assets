...
namespace GameLogic.Navigation{
	...
	/// 一个二进制堆，用于数据排序和优先级队列
	/// </summary>: 
	public class NBinaryHeap<T> : ICollection<T> where T : IComparable<T>{
		#region Constants
		...
		#region Opreation Methods
		public NBinaryHeap(){
		}

		private NBinaryHeap(T[] data,int count){
			Capacity = count;
			_count = count;
		...
		public NBinaryHeap<T> Copy()
		{
			return new NBinaryHeap<T>(_data, _count);
		}
